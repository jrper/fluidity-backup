...
subroutine test_supermesh_shapes_bc
  ...
  integer :: degree, i
  real, dimension(:), allocatable :: shape_rhs_integral_b, shape_rhs_integral_c
  real, dimension(:, :), allocatable :: dshape_dot_dshape_integral_b, dshape_dot_dshape_integral_c
  type(element_type) :: shape_b, shape_c
  type(element_type), dimension(:), allocatable :: shapes_c
  type(mesh_type) :: mesh_b, mesh_c, shape_mesh
  type(quadrature_type) :: quad
  type(vector_field) :: positions_b, positions_c
  
  do degree = 0, 4
    print *, "Degree = ", degree
  
    quad = make_quadrature(vertices = 3, dim  = 2, degree = max(degree * 2, 1))
    shape_b = make_element_shape(vertices = 3, dim  = 2, degree = 1, quad = quad)
    shape_c = make_element_shape(vertices = 3, dim  = 2, degree = degree, quad = quad)
    call deallocate(quad)
    
    call allocate(mesh_b, nodes = 3, elements = 1, shape = shape_b, name = "TargetMesh")
    call set_ele_nodes(mesh_b, 1, (/1, 2, 3/))
    
    call allocate(positions_b, dim = 2, mesh = mesh_b, name = "TargetCoordinate")
    call deallocate(mesh_b)
    call set(positions_b, 1, (/0.0, 0.0/))
    call set(positions_b, 2, (/1.0, 0.0/))
    call set(positions_b, 3, (/0.0, 1.0/))
    
    call allocate(mesh_c, nodes = 4, elements = 3, shape = shape_b, name = "Supermesh")
    call deallocate(shape_b)
    ...
    shape_mesh = make_mesh(positions_b%mesh, shape = shape_c, continuity = -1, name = "ShapeMesh")
    call deallocate(shape_c)
    
    call project_target_shape_to_supermesh(1, &
      & positions_b, shape_mesh, positions_c, &
      & shapes_c, form_dn = .true.)
      
    allocate(shape_rhs_integral_b(ele_loc(shape_mesh, 1)))
    shape_rhs_integral_b = shape_rhs_integral_ele(1, positions_b, ele_shape(shape_mesh, 1))
    ...
    call report_test("[shape_rhs on supermesh]", shape_rhs_integral_b .fne. shape_rhs_integral_c, .false., "Incorrect integral")
      
    deallocate(shape_rhs_integral_b)
    deallocate(shape_rhs_integral_c)
    
    allocate(dshape_dot_dshape_integral_b(ele_loc(shape_mesh, 1), ele_loc(shape_mesh, 1)))
    dshape_dot_dshape_integral_b = dshape_dot_dshape_integral_ele(1, positions_b, ele_shape(shape_mesh, 1))
    ...
    call report_test("[dshape_dot_dshape on supermesh]", fnequals(dshape_dot_dshape_integral_b, dshape_dot_dshape_integral_c, tol = 1.0e3 * epsilon(0.0)), .false., "Incorrect integral")
    
    deallocate(dshape_dot_dshape_integral_b)
...
end subroutine test_supermesh_shapes_bc
