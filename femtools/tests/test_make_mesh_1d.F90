...
subroutine test_make_mesh_1d
  ...
  call report_test("[1-simplex input mesh]", &
    & ele_numbering_family(base_shape) /= FAMILY_SIMPLEX .or. base_shape%degree /= 1 .or. base_shape%dim /= 1, .false., &
    & "Input mesh not composed of 1-simplices")
  ...
  do degree = min_degree, max_degree
    ...
    otn_l_coords = d1_otn_local_coords(degree)
...
contains
  ...
  function d1_otn_local_coords(degree) result(l_coords)
    !!< Return the node local coords according to the One True Element Numbering
  
    integer, intent(in) :: degree
    
    integer :: i, index
    real, dimension(2, degree + 1) :: l_coords
        
    index = 1
    do i = 0, degree
      assert(index <= size(l_coords, 2))
      l_coords(2, index) = float(i) / float(degree)
      l_coords(1, index) = 1.0 - l_coords(2, index)
      index = index + 1
    end do
    assert(index == size(l_coords, 2) + 1)
    
  end function d1_otn_local_coords

end subroutine test_make_mesh_1d
