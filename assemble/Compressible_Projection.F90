  ...
  subroutine assemble_compressible_projection_cv(state, cmc, rhs, dt, theta_pg, theta_divergence, cmcget)
    ...
    if((size(state)==1).and.(.not.has_scalar_field(state(1), "MaterialVolumeFraction"))) then
      ...
      call assemble_1mat_compressible_projection_cv(state(1), cmc, rhs, dt, &
       ...
       if (have_option(trim(state(1)%option_path)//&
            &'/equation_of_state/compressible/ATHAM')) then
          call assemble_atham_compressible_projection_cv(state, cmc, rhs, dt, &
                                                    theta_pg, theta_divergence, cmcget)
       else
  ...
  subroutine assemble_atham_compressible_projection_cv(state, cmc, rhs, dt, &
    ...
    if(cmcget) then
      ...
      call get_option(trim(pressure%option_path)//"/prognostic/scheme/use_compressible_projection_method/normalisation/name", &
                      normalisation_field, stat=norm_stat)
      if(norm_stat==0) then
        normalisation=>extract_scalar_field(state(1), trim(normalisation_field))
      else
        allocate(normalisation)
        call allocate(normalisation, pressure%mesh, "DummyNormalisation", field_type=FIELD_TYPE_CONSTANT)
        call set(normalisation, 1.0)
...
!      call addto(rhs, atmospheric_pressure)
      ...
      source => extract_scalar_field(state(1), "DensitySource", stat=stat)
      if(stat==0) then
        call addto(rhs, source)
      end if
      
      absorption => extract_scalar_field(state(1), "DensityAbsorption", stat=stat)
      if(stat==0) then
        call allocate(absrhs, absorption%mesh, "AbsorptionRHS")
  ...
  end subroutine assemble_atham_compressible_projection_cv
  ...
  subroutine assemble_compressible_projection_cg(state, cmc, rhs, dt, theta_pg, theta_divergence, cmcget)
    ...
    if((size(state)==1).and.(.not.has_scalar_field(state(1), "MaterialVolumeFraction"))) then
      ...
      call assemble_1mat_compressible_projection_cg(state(1), cmc, rhs, dt, &
       ...
       if (have_option(trim(state(1)%option_path)//&
            &'/equation_of_state/compressible/ATHAM')) then
          call assemble_atham_compressible_projection_cg(state, cmc, rhs, dt, &
               theta_pg, theta_divergence, cmcget)
       else
...
!! end bc routine
  ...
  subroutine assemble_atham_compressible_projection_cg(state, cmc, rhs, dt, &
                                                      theta_pg, theta_divergence, cmcget)
    ...
    ewrite(1,*) 'Entering assemble_atham_compressible_projection_cg'
    
    call zero(rhs)

    ! only do all this if we need to make cmc (otherwise we'd be adding repeatedly)
    if(cmcget) then
      coordinate=> extract_vector_field(state(1), "Coordinate",stat=stat)
      if (stat /= 0)&
         print*, "No coordinate field in "//state(1)%name//"!"
      
      density => extract_scalar_field(state(1), "Density",stat=stat)
      if (stat/=0) &
           print*, "No density field in "//state(1)%name//"!"
      olddensity => extract_scalar_field(state(1), "OldDensity",stat=stat)
      if (stat/=0) &
           print*, "No old density field in "//state(1)%name//"!"
      ...
      if (stat/=0) &
         print*, "No velocity field in "//state(1)%name//"!"
      nonlinearvelocity=>extract_vector_field(state(1), "NonlinearVelocity") ! maybe this should be updated after the velocity solve?
      
      pressure => extract_scalar_field(state(1), "Pressure",stat=stat)
      if (stat/=0) &
           print*, "No pressure field in "//state(1)%name//"!"
 ...
 end subroutine assemble_atham_compressible_projection_cg
  ...
  subroutine update_compressible_density(state)
   ...
   elseif (have_option(trim(state(1)%option_path)//'/equation_of_state/compressible/ATHAM')) then
      density=>extract_scalar_field(state(1),'Density')
  ...
  subroutine compressible_initialise(state)
    ...
    sfield=>extract_scalar_field(state(1),"InternalEnergy",stat)
    if (stat/=0) &
