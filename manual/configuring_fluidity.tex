...
\ref{sec:coriolis}) in a number of different ways. Note that only the  \option{specified\_axis} option will include the centrifugal acceleration term explicitly. The following options
are available: 
...
\begin{enumerate}
 ...
 \item \option{specified\_axis} -- specify both the angular speed of the rotation, $\Omega$ and a point on the axis around which the rotation occurs (still assumed to be parallel to the $\bm{z}$ axis). As stated above, this adds the centrifugal force to the body forces applied in the simulation.
\end{enumerate}
